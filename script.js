/* ============================================================================
   EDGE QUIZ BUILDER - script.js
   ============================================================================

   PURPOSE
   -------
   Main browser-side logic for the EDGE Quiz Builder. This file controls:
   - rich-text authoring and formatting
   - question/page generation and navigation
   - learner-size overflow measurement and validation
   - JSON / standalone HTML import and export
   - learner preview generation
   - Word export
   - autosave / restore
   - builder viewport scaling

   MAINTENANCE MAP
   ---------------
   01. Builder state and shared constants
   02. Rich-text selection and toolbar formatting
   03. Shared HTML editor
   04. Field HTML storage and line spacing
   05. Learner-size fit measurement
   06. Builder page generation, navigation and feedback controls
   07. Required-field validation and export-data construction
   08. Preview, JSON import/export and standalone ZIP export
   09. Import normalisation (including MathML)
   10. General utilities and legacy preview
   11. AI prompt helper
   12. Word export
   13. Learner preview HTML builder
   14. Platform-compatible standalone export builder
   15. Autosave and full builder validation
   16. Builder viewport fit and startup wiring

   IMPORTANT - EXPORT SAFETY
   -------------------------
   buildStandaloneQuizHtml() contains the platform-compatible learner export.
   Treat that section as protected. Changes there can affect package/runtime,
   scoring, completion callbacks, randomisation and LMS behaviour.

   The builder may be changed independently, but learner-export behaviour
   should only be changed intentionally and should always be regression-tested.

   IMPORTANT - LAYOUT / VALIDATION
   -------------------------------
   learnerMeasurementProfile() mirrors fixed learner dimensions. If learner
   question/answer/feedback dimensions change, update the matching layout and
   validation measurements together or fit results will no longer be reliable.

   NOTE
   ----
   This documented version preserves the original executable code and order.
   Comments were added for maintainability; functional logic was not rewritten.
   ============================================================================ */
...
/* ============================================================================
   01. BUILDER STATE + SHARED CONSTANTS
   Runtime state used by the authoring UI. Most of these values are reset when
   question fields are regenerated or when the active page/editor changes.
   ============================================================================ */
...
/* Commands represented by buttons in the shared rich-text toolbar. */
...
/* Block-level tags recognised when normalising HTML entered/pasted by users. */
const HTML_BLOCK_TAGS = new Set([
  "P", "DIV", "UL", "OL", "LI", "BLOCKQUOTE", "PRE",
  "H1", "H2", "H3", "H4", "H5", "H6", "TABLE"
]);

/* Supported per-container line-height values stored with quiz HTML. */
...
/* ============================================================================
   02. RICH-TEXT SELECTION + TOOLBAR FORMATTING
   Registers contenteditable fields, tracks the active selection, reports
   toolbar state and applies inline formatting without losing the selection.
   ============================================================================ */

/** Attach authoring/formatting/overflow listeners to one editable field. */
...
/** Return text nodes intersecting the current selection inside an editor. */
...
/** Determine whether a text node currently has a toolbar format applied. */
...
/** Refresh active toolbar buttons, font-size control and line-spacing control. */
...
/** Wrap the selected content in a styled span while preserving selection. */
...
/** Apply a bold/italic/underline/list/super/sub formatting command. */
...
/* ============================================================================
   03. SHARED HTML EDITOR
   Keeps raw HTML and visual contenteditable fields synchronised. The normaliser
   converts loose top-level inline content/divs into predictable paragraph flow.
   ============================================================================ */

/** Normalise raw field HTML before it is shown in the side HTML editor. */
...
/** Copy the raw HTML editor value back into the visible authoring field. */
...
/* ============================================================================
   04. FIELD HTML STORAGE + LINE SPACING
   Quiz content is stored as HTML. Container-level line spacing is serialised
   with data-qb-line-spacing so JSON/import/export can restore it consistently.
   ============================================================================ */

/** Split stored wrapper HTML into content plus its saved line-spacing value. */
...
/** Read one authoring field as serialisable HTML with line-spacing metadata. */
function getHtml(id) {
...
/** Populate one authoring field from previously stored/imported HTML. */
...
/* ============================================================================
   05. LEARNER-SIZE FIT MEASUREMENT
   Uses hidden off-screen boxes with the REAL learner dimensions rather than the
   scaled builder canvas. This is the source of truth for overflow validation.
   ============================================================================ */

/** Return the fixed learner dimensions/typography for an editable field type. */
...
/** Build an off-screen learner-sized copy of a field for accurate measuring. */
...
/** Measure vertical/horizontal text overflow in learner pixels. */
...
/** Detect images/SVG/video/etc. that exceed the learner content area. */
...
/* ============================================================================
   06. BUILDER PAGES + NAVIGATION + FEEDBACK VIEW
   Generates the editable title/question canvases, question tabs and controls
   visibility when the author moves through the builder.
   ============================================================================ */

/** Return the authoring markup for the title/start screen. */
...
/** Return the authoring markup for one question page. */
function buildQuestionPage(i,count){
  return `<div class="question-block" id="questionPage_${i}"><div class="quiz-canvas builder-question">
    ...
    <div class="question-image-mock"><img src="img/image${((i-1)%5)+1}.png" alt="Question illustration"></div>
    <button type="button" class="feedback-toggle" onclick="toggleBuilderFeedback()">Feedback view</button>
    <div class="feedback-mock" style="display:none"><div class="feedback-avatar-mock"><img src="img/correct${((i-1)%5)+1}.png" alt="Feedback illustration"></div><div class="feedback-label-mock">CORRECT</div>
...
/** Rebuild title/question authoring pages for the selected question count. */
...
/* ============================================================================
   07. REQUIRED-FIELD VALIDATION + EXPORT DATA MODEL
   validateQuizForm() is the basic blocking check used before export. The richer
   validation system later in this file provides detailed fit/content feedback.
   ============================================================================ */
...
/** Build the canonical quiz data object shared by preview and all exports. */
...
/* ============================================================================
   08. PREVIEW + JSON / STANDALONE IMPORT-EXPORT
   Preview uses the current builder data. JSON is the editable interchange
   format. Standalone export creates the LMS package using the protected runtime.
   ============================================================================ */
...
// Builder, Preview and Export now share the single img/ folder.
...
/** Package story.html plus all required learner PNG assets into a ZIP. */
...
/** Import either Quiz Builder JSON or a previously exported standalone HTML. */
...
/* ============================================================================
   09. IMPORT NORMALISATION
   Cleans imported rich text and standardises MathML display alignment/width so
   Builder, Preview and Export interpret imported equations consistently.
   ============================================================================ */
...
/** Normalise imported HTML/MathML without changing the mathematical meaning. */
...
/* ============================================================================
   10. GENERAL UTILITIES + LEGACY IN-PAGE PREVIEW
   Small browser helpers followed by the older preview path retained for
...
/* ============================================================================
   11. AI CONVERSION PROMPT HELPER
   Controls the Fast / High-Fidelity prompt selector and clipboard copy action.
   ============================================================================ */
...
/* ============================================================================
   12. WORD EXPORT
   Converts supported HTML formatting to docx TextRuns and builds a reviewable
   Word document. Word import is intentionally not supported in-browser.
   ============================================================================ */
...
/* ============================================================================
   13. LEARNER PREVIEW HTML BUILDER
   Creates the local learner preview in a new window. It contains its own
   embedded learner CSS/runtime so preview can be tested independently.
   ============================================================================ */

// ── Current learner preview HTML builder ──────────────────

/** Build the complete local preview HTML document as a string. */
function buildPreviewQuizHtml(exportData, title, baseUrl) {
  ...
  const exportedFolderName = String(exportData.filename || "").trim().replace(/[^a-zA-Z0-9_-]/g, "_") || "quiz";
  const safeBaseUrl = String(baseUrl || "").replace(/&/g, "&amp;").replace(/"/g, "&quot;");
  const runtime = `
"use strict";
["copy","cut","selectstart","dragstart"].forEach(type=>{document.addEventListener(type,event=>event.preventDefault(),{capture:true})});
...
`.replace("__DATA__", JSON.stringify(exportData.questions))
  ...
  return `<!DOCTYPE html><html lang="en"><head><base href="${safeBaseUrl}"><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>${exportedFolderName}</title><meta name="quiz-title" content="${String(title||"").replace(/"/g,"&quot;")}"><meta name="quiz-filename" content="${safeFilename}"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500&display=swap" rel="stylesheet"><style>:root {
  --slide-width: 720px;
  --slide-height: 540px;
  --navy: #003770;
  --title-colour: #00273b;
...
/* Keep MathML display equations intrinsic-width, matching the builder. */
.question-box math,
#answersContainer math,
.feedback-text math {
  vertical-align: baseline;
}

.question-box math[display="block"],
#answersContainer math[display="block"],
.feedback-text math[display="block"] {
  display: block;
  width: max-content;
  max-width: 100%;
  min-width: 0;
...
.question-box math[display="block"][data-qb-align="center"],
#answersContainer math[display="block"][data-qb-align="center"],
.feedback-text math[display="block"][data-qb-align="center"] {
  margin-left: auto;
  margin-right: auto;
  text-align: center;
}

.question-box math[display="block"][data-qb-align="right"],
#answersContainer math[display="block"][data-qb-align="right"],
.feedback-text math[display="block"][data-qb-align="right"] {
  margin-left: auto;
  margin-right: 0;
  text-align: right;
}
...
/* ============================================================================
   14. PLATFORM-COMPATIBLE STANDALONE EXPORT - PROTECTED AREA

   WARNING FOR FUTURE MAINTAINERS
   ------------------------------
   This section produces story.html for the LMS/platform package. It includes
   platform callbacks, scoring/completion behaviour, randomisation semantics,
   review behaviour and fixed learner markup/CSS.

   Do not refactor this section merely for style/readability. Any functional
   change should be deliberate and regression-tested against a known-good export.
   ============================================================================ */

// ── Platform-compatible export HTML builder ───────────────

/** Build the platform-facing story.html document used in the exported ZIP. */
...
/* ============================================================================
   15. AUTOSAVE + FULL BUILDER VALIDATION
   Saves authoring state to localStorage and produces the detailed validation
   report for missing content, overflow, visuals and readability warnings.
   ============================================================================ */
...
/** Collect all validation issues and return them in severity order. */
...
/** Navigate directly from a validation result to the affected builder field. */
...
/* ============================================================================
   16. BUILDER VIEWPORT FIT + STARTUP
   Scales only the authoring canvas to fit the browser. The learner/export size
   remains 720 x 540. DOMContentLoaded wires controls, restores autosave and
   starts resize observers.
   ============================================================================ */
...
/** Recalculate the visual authoring scale without changing learner dimensions. */
...
/** Application bootstrap: register UI behaviour, generate fields and restore draft. */
